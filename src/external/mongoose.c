...
MG_INTERNAL void mg_call(struct mg_connection *nc, mg_event_handler_t ev_handler, void *user_data, int ev,
						 void *ev_data);
void mg_forward(struct mg_connection *from, struct mg_connection *to);
MG_INTERNAL void mg_add_conn(struct mg_mgr *mgr, struct mg_connection *c);
MG_INTERNAL void mg_remove_conn(struct mg_connection *c);
MG_INTERNAL struct mg_connection *mg_create_connection(struct mg_mgr *mgr, mg_event_handler_t callback,
...
MG_INTERNAL struct mg_http_proto_data *mg_http_create_proto_data(struct mg_connection *c);
...
MG_INTERNAL int mg_uri_to_local_path(struct http_message *hm, const struct mg_serve_http_opts *opts, char **local_path,
									 struct mg_str *remainder);
MG_INTERNAL time_t mg_parse_date_string(const char *datetime);
MG_INTERNAL int mg_is_not_modified(struct http_message *hm, cs_stat_t *st);
#endif
#if MG_ENABLE_HTTP_CGI
MG_INTERNAL void mg_handle_cgi(struct mg_connection *nc, const char *prog, const struct mg_str *path_info,
							   const struct http_message *hm, const struct mg_serve_http_opts *opts);
...
MG_INTERNAL void mg_handle_ssi_request(struct mg_connection *nc, struct http_message *hm, const char *path,
									   const struct mg_serve_http_opts *opts);
#endif
#if MG_ENABLE_HTTP_WEBDAV
MG_INTERNAL int mg_is_dav_request(const struct mg_str *s);
MG_INTERNAL void mg_handle_propfind(struct mg_connection *nc, const char *path, cs_stat_t *stp, struct http_message *hm,
...
MG_INTERNAL void mg_handle_move(struct mg_connection *c, const struct mg_serve_http_opts *opts, const char *path,
...
int cs_base64_decode(const unsigned char *s, int len, char *dst, int *dec_len)
{
	...
	int orig_len = len;
	char *orig_dst = dst;
	while (len >= 4 && (a = from_b64(s[0])) != 255 && (b = from_b64(s[1])) != 255 && (c = from_b64(s[2])) != 255 &&
		   (d = from_b64(s[3])) != 255) {
		s += 4;
		len -= 4;
		if (a == 200 || b == 200)
...
void cs_hmac_sha1(const unsigned char *key, size_t keylen, const unsigned char *data, size_t datalen,
...
#define _MG_ALLOWED_CONNECT_FLAGS_MASK                                                                              \
	(MG_F_USER_1 | MG_F_USER_2 | MG_F_USER_3 | MG_F_USER_4 | MG_F_USER_5 | MG_F_USER_6 | MG_F_WEBSOCKET_NO_DEFRAG | \
	 MG_F_ENABLE_BROADCAST)
/* Which flags should be modifiable by user's callbacks. */
#define _MG_CALLBACK_MODIFIABLE_FLAGS_MASK                                                                          \
	(MG_F_USER_1 | MG_F_USER_2 | MG_F_USER_3 | MG_F_USER_4 | MG_F_USER_5 | MG_F_USER_6 | MG_F_WEBSOCKET_NO_DEFRAG | \
	 MG_F_SEND_AND_CLOSE | MG_F_CLOSE_IMMEDIATELY | MG_F_IS_WEBSOCKET | MG_F_DELETE_CHUNK)
						 ...
						 void *ev_data)
{
	...
	if (ev != MG_EV_POLL) {
		DBG(("%p %s ev=%d ev_data=%p flags=0x%lx rmbl=%d smbl=%d", nc, ev_handler == nc->handler ? "user" : "proto", ev,
	...
	if (ev != MG_EV_POLL)
		nc->mgr->num_calls++;
	if (ev != MG_EV_POLL) {
		DBG(("%p after %s flags=0x%lx rmbl=%d smbl=%d", nc, ev_handler == nc->handler ? "user" : "proto", nc->flags,
...
MG_INTERNAL struct mg_connection *mg_create_connection_base(struct mg_mgr *mgr, mg_event_handler_t callback,
															struct mg_add_sock_opts opts)
{
...
{
	LOG(LL_DEBUG, ("%p %s://%s:%hu", nc, (nc->flags & MG_F_UDP ? "udp" : "tcp"), inet_ntoa(sa->sin.sin_addr),
...
MG_INTERNAL struct mg_connection *mg_do_connect(struct mg_connection *nc, int proto, union socket_address *sa)
{
	LOG(LL_DEBUG, ("%p %s://%s:%hu", nc, proto == SOCK_DGRAM ? "udp" : "tcp", inet_ntoa(sa->sin.sin_addr),
				   ntohs(sa->sin.sin_port)));
...
{
	LOG(LL_DEBUG, ("%p %s://%s:%hu -> %d", nc, (nc->flags & MG_F_UDP ? "udp" : "tcp"), inet_ntoa(nc->sa.sin.sin_addr),
				   ntohs(nc->sa.sin.sin_port), err));
	nc->flags &= ~MG_F_CONNECTING;
	if (err != 0) {
		nc->flags |= MG_F_CLOSE_IMMEDIATELY;
	}
...
struct mg_connection *mg_connect_opt(struct mg_mgr *mgr, const char *address,
									 MG_CB(mg_event_handler_t callback, void *user_data), struct mg_connect_opts opts)
{
...
#if MG_ENABLE_SSL
	LOG(LL_DEBUG, ("%p %s %s,%s,%s", nc, address, (opts.ssl_cert ? opts.ssl_cert : "-"),
				   (opts.ssl_key ? opts.ssl_key : "-"), (opts.ssl_ca_cert ? opts.ssl_ca_cert : "-")));
...
#if MG_ENABLE_SSL
	DBG(("%p %s %s,%s,%s", nc, address, (opts.ssl_cert ? opts.ssl_cert : "-"), (opts.ssl_key ? opts.ssl_key : "-"),
...
#define MG_NULL_IFACE_VTABLE                                                                                \
	{                                                                                                       \
		mg_null_if_init, mg_null_if_free, mg_null_if_add_conn, mg_null_if_remove_conn, mg_null_if_poll,     \
			mg_null_if_listen_tcp, mg_null_if_listen_udp, mg_null_if_connect_tcp, mg_null_if_connect_udp,   \
			mg_null_if_tcp_send, mg_null_if_udp_send, mg_null_if_tcp_recv, mg_null_if_udp_recv,             \
			mg_null_if_create_conn, mg_null_if_destroy_conn, mg_null_if_sock_set, mg_null_if_get_conn_addr, \
	}
...
void mg_socket_if_connect_tcp(struct mg_connection *nc, const union socket_address *sa)
{
	...
	DBG(("%p sock %d rc %d errno %d err %d", nc, (int)nc->sock, rc, mg_get_errno(), nc->err));
}
...
static int mg_socket_if_udp_recv(struct mg_connection *nc, void *buf, size_t len, union socket_address *sa,
								 size_t *sa_len)
{
...
void mg_mgr_handle_conn(struct mg_connection *nc, int fd_flags, double now)
{
	...
	if (worth_logging) {
		DBG(("%p fd=%d fd_flags=%d nc_flags=0x%lx rmbl=%d smbl=%d", nc, (int)nc->sock, fd_flags, nc->flags,
			 (int)nc->recv_mbuf.len, (int)nc->send_mbuf.len));
	}
	...
	if (worth_logging) {
		DBG(("%p after fd=%d nc_flags=0x%lx rmbl=%d smbl=%d", nc, (int)nc->sock, nc->flags, (int)nc->recv_mbuf.len,
...
int mg_socketpair(sock_t sp[2], int sock_type)
{
	...
	} else if (sock_type == SOCK_DGRAM && (bind(sp[0], &sa2.sa, len) != 0 || getsockname(sp[0], &sa2.sa, &len) != 0 ||
										   connect(sp[0], &sa.sa, len) != 0 || connect(sock, &sa2.sa, len) != 0)) {
...
const struct mg_iface_vtable mg_socks_iface_vtable = {
	mg_socks_if_init,          mg_socks_if_free,        mg_socks_if_add_conn,     mg_socks_if_remove_conn,
	mg_socks_if_poll,          mg_socks_if_listen_tcp,  mg_socks_if_listen_udp,   mg_socks_if_connect_tcp,
	mg_socks_if_connect_udp,   mg_socks_if_tcp_send,    mg_socks_if_udp_send,     mg_socks_if_tcp_recv,
	mg_socks_if_udp_recv,      mg_socks_if_create_conn, mg_socks_if_destroy_conn, mg_socks_if_sock_set,
...
static enum mg_ssl_if_result mg_ssl_if_ossl_set_psk(struct mg_ssl_if_ctx *ctx, const char *identity,
													const char *key_str);
...
{
	struct mg_ssl_if_ctx *ctx = (struct mg_ssl_if_ctx *)MG_CALLOC(1, sizeof(*ctx));
	DBG(("%p %s,%s,%s", nc, (params->cert ? params->cert : ""), (params->key ? params->key : ""),
...
static unsigned int mg_ssl_if_ossl_psk_cb(SSL *ssl, const char *hint, char *identity, unsigned int max_identity_len,
...
static enum mg_ssl_if_result mg_use_cert(struct mg_ssl_if_ctx *ctx, const char *cert, const char *key,
										 const char **err_msg);
...
#ifdef MBEDTLS_KEY_EXCHANGE__SOME__PSK_ENABLED
static enum mg_ssl_if_result mg_ssl_if_mbed_set_psk(struct mg_ssl_if_ctx *ctx, const char *identity,
													const char *key_str)
{
	unsigned char key[32];
	size_t key_len;
	...
	if (mbedtls_ssl_conf_psk(ctx->conf, (const unsigned char *)key, key_len, (const unsigned char *)identity,
							 strlen(identity)) != 0) {
		return MG_SSL_ERROR;
	}
	return MG_SSL_OK;
...
{
	struct mg_str rscheme = {0, 0}, ruser_info = {0, 0}, rhost = {0, 0}, rpath = {0, 0}, rquery = {0, 0},
				  rfragment = {0, 0};
	unsigned int rport = 0;
	enum { P_START, P_SCHEME_OR_PORT, P_USER_INFO, P_HOST, P_PORT, P_REST } state = P_START;
...
int mg_assemble_uri(const struct mg_str *scheme, const struct mg_str *user_info, const struct mg_str *host,
					unsigned int port, const struct mg_str *path, const struct mg_str *query,
					const struct mg_str *fragment, int normalize_path, struct mg_str *uri)
{
	int result = -1;
	struct mbuf out;
	mbuf_init(&out, 0);
...
static struct mg_str mg_get_mime_types_entry(struct mg_str path)
{
	...
	for (i = 0; mg_static_builtin_mime_types[i].extension != NULL; i++) {
		if (path.len < mg_static_builtin_mime_types[i].ext_len + 1)
			continue;
		struct mg_str ext =
			MG_MK_STR_N(mg_static_builtin_mime_types[i].extension, mg_static_builtin_mime_types[i].ext_len);
...
MG_INTERNAL int mg_get_mime_type_encoding(struct mg_str path, struct mg_str *type, struct mg_str *encoding,
										  const struct mg_serve_http_opts *opts)
{
	...
	*type = mg_get_mime_types_entry(path);
	...
	if (mg_vcmp(type, "application/x-gunzip") == 0) {
		...
		LOG(LL_ERROR,
			("'%.*s' '%.*s' '%.*s'", (int)path.len, path.p, (int)path2.len, path2.p, (int)type2.len, type2.p));
		if (type2.len > 0) {
			*type = type2;
...
int mg_parse_http(const char *s, int n, struct http_message *hm, int is_req)
{
   ...
   */
	if (hm->body.len == (size_t)~0 && is_req && mg_vcasecmp(&hm->method, "PUT") != 0 &&
...
static void mg_http_transfer_file_data(struct mg_connection *nc)
{
	...
	if (pd->file.type == DATA_FILE) {
		...
		if (pd->file.sent >= pd->file.cl) {
			LOG(LL_DEBUG, ("%p done, %d bytes, ka %d", nc, (int)pd->file.sent, pd->file.keepalive));
 ...
 */
#ifdef __xtensa__
static void mg_http_handler2(struct mg_connection *nc, int ev, void *ev_data MG_UD_ARG(void *user_data),
...
void mg_http_handler(struct mg_connection *nc, int ev, void *ev_data MG_UD_ARG(void *user_data))
{
	...
	if (ev == MG_EV_CLOSE) {
       ...
       */
			...
			mg_call(nc, (pd->endpoint_handler ? pd->endpoint_handler : nc->handler), nc->user_data, MG_EV_HTTP_PART_END,
					&mp);
			mp.var_name = NULL;
			mp.file_name = NULL;
			mg_call(nc, (pd->endpoint_handler ? pd->endpoint_handler : nc->handler), nc->user_data,
       ...
       */
		...
		if (pd != NULL && pd->endpoint_handler != NULL && pd->endpoint_handler != nc->handler) {
...
#if MG_ENABLE_HTTP_STREAMING_MULTIPART
	if (pd != NULL && pd->mp_stream.boundary != NULL && (ev == MG_EV_RECV || ev == MG_EV_POLL)) {
		if (ev == MG_EV_RECV) {
			pd->rcvd += *(int *)ev_data;
	...
	again:
...
#if MG_ENABLE_HTTP_STREAMING_MULTIPART
		if (req_len > 0 && (s = mg_get_http_header(hm, "Content-Type")) != NULL && s->len >= 9 &&
			strncmp(s->p, "multipart", 9) == 0) {
		...
		if ((req_len < 0 || (req_len == 0 && io->len >= MG_MAX_HTTP_REQUEST_SIZE))) {
			...
			if (hm->resp_code == 101 && mg_get_http_header(hm, "Sec-WebSocket-Accept")) {
				/* TODO(lsm): check the validity of accept Sec-WebSocket-Accept */
				mg_call(nc, nc->handler, nc->user_data, MG_EV_WEBSOCKET_HANDSHAKE_DONE, hm);
       ...
       */
			...
			if (nc->recv_mbuf_limit > 0 && nc->recv_mbuf.len >= nc->recv_mbuf_limit) {
				LOG(LL_ERROR, ("%p recv buffer (%lu bytes) exceeds the limit "
							   ...
							   nc, (unsigned long)nc->recv_mbuf.len, (unsigned long)nc->recv_mbuf_limit));
				nc->flags |= MG_F_CLOSE_IMMEDIATELY;
			}
		} else {
...
static size_t mg_http_multipart_call_handler(struct mg_connection *c, int ev, const char *data, size_t data_len)
{
	struct mg_http_multipart_part mp;
...
static int mg_http_multipart_continue_wait_for_chunk(struct mg_connection *c)
{
	...
	if (boundary == NULL) {
		...
		if (data_len > 0) {
			size_t consumed = mg_http_multipart_call_handler(c, MG_EV_HTTP_PART_DATA, io->buf, (size_t)data_len);
			mbuf_remove(io, consumed);
		}
		return 0;
	} else if (boundary != NULL) {
		size_t data_len = ((size_t)(boundary - io->buf) - 4);
		size_t consumed = mg_http_multipart_call_handler(c, MG_EV_HTTP_PART_DATA, io->buf, data_len);
		mbuf_remove(io, consumed);
						   ...
						   const struct mg_str extra_headers)
{
	...
	mg_asprintf(&phead, sizeof(bhead),
				...
				"%.*s%s",
				(int)location.len, location.p, bl, (int)extra_headers.len, extra_headers.p,
				(extra_headers.len > 0 ? "\r\n" : ""));
...
void mg_http_serve_file_internal(struct mg_connection *nc, struct http_message *hm, const char *path,
								 struct mg_str mime_type, struct mg_str encoding, struct mg_str extra_headers)
{
	struct mg_http_proto_data *pd = mg_http_get_proto_data(nc);
	cs_stat_t st;
	LOG(LL_DEBUG, ("%p [%s] %.*s %.*s", nc, path, (int)mime_type.len, mime_type.p, (int)encoding.len, encoding.p));
	if (mg_stat(path, &st) != 0 || (pd->file.fp = mg_fopen(path, "rb")) == NULL) {
...
#if _FILE_OFFSET_BITS == 64 || _POSIX_C_SOURCE >= 200112L || _XOPEN_SOURCE >= 600
		...
		mg_printf(nc,
				  ...
				  (pd->file.keepalive ? "keep-alive" : "close"), (size_t)cl, range, etag);
		if (encoding.len > 0) {
...
{
	mg_http_serve_file_internal(nc, hm, path, mime_type, mg_mk_str(NULL), extra_headers);
}

static void mg_http_serve_file2(struct mg_connection *nc, const char *path, struct http_message *hm,
								struct mg_serve_http_opts *opts)
{
#if MG_ENABLE_HTTP_SSI
	...
	mg_http_serve_file_internal(nc, hm, path, type, encoding, mg_mk_str(opts->extra_headers));
}
...
static void mg_mkmd5resp(const char *method, size_t method_len, const char *uri, size_t uri_len, const char *ha1,
						 size_t ha1_len, const char *nonce, size_t nonce_len, const char *nc, size_t nc_len,
						 const char *cnonce, size_t cnonce_len, const char *qop, size_t qop_len, char *resp)
{
	static const char colon[] = ":";
	static const size_t one = 1;
	char ha2[33];
	cs_md5(ha2, method, method_len, colon, one, uri, uri_len, NULL);
	cs_md5(resp, ha1, ha1_len, colon, one, nonce, nonce_len, colon, one, nc, nc_len, colon, one, cnonce, cnonce_len,
		   colon, one, qop, qop_len, colon, one, ha2, sizeof(ha2) - 1, NULL);
}

int mg_http_create_digest_auth_header(char *buf, size_t buf_len, const char *method, const char *uri,
									  const char *auth_domain, const char *user, const char *passwd, const char *nonce)
{
	...
	cs_md5(ha1, user, (size_t)strlen(user), colon, one, auth_domain, (size_t)strlen(auth_domain), colon, one, passwd,
		   (size_t)strlen(passwd), NULL);
	mg_mkmd5resp(method, strlen(method), uri, strlen(uri), ha1, sizeof(ha1) - 1, nonce, strlen(nonce), "1", one, cnonce,
...
int mg_http_check_digest_auth(struct http_message *hm, const char *auth_domain, FILE *fp)
{
	...
	char *username = username_buf, *cnonce = cnonce_buf, *response = response_buf, *uri = uri_buf, *qop = qop_buf,
		 *nc = nc_buf, *nonce = nonce_buf;

	/* Parse "Authorization:" header, fail fast on parse error */
	if (hm == NULL || fp == NULL || (hdr = mg_get_http_header(hm, "Authorization")) == NULL ||
		mg_http_parse_header2(hdr, "username", &username, sizeof(username_buf)) == 0 ||
		mg_http_parse_header2(hdr, "cnonce", &cnonce, sizeof(cnonce_buf)) == 0 ||
		mg_http_parse_header2(hdr, "response", &response, sizeof(response_buf)) == 0 ||
		...
		mg_http_parse_header2(hdr, "nonce", &nonce, sizeof(nonce_buf)) == 0 || mg_check_nonce(nonce) == 0) {
		ret = 0;
		goto clean;
	}
	...
	ret = mg_check_digest_auth(
		hm->method, mg_mk_str_n(hm->uri.p, hm->uri.len + (hm->query_string.len ? hm->query_string.len + 1 : 0)),
		mg_mk_str(username), mg_mk_str(cnonce), mg_mk_str(response), mg_mk_str(qop), mg_mk_str(nc), mg_mk_str(nonce),
		mg_mk_str(auth_domain), fp);
...
int mg_check_digest_auth(struct mg_str method, struct mg_str uri, struct mg_str username, struct mg_str cnonce,
						 struct mg_str response, struct mg_str qop, struct mg_str nc, struct mg_str nonce,
						 struct mg_str auth_domain, FILE *fp)
{
   ...
   */
	while (fgets(buf, sizeof(buf), fp) != NULL) {
		if (sscanf(buf, "%[^:]:%[^:]:%s", f_user, f_domain, f_ha1) == 3 && mg_vcmp(&username, f_user) == 0 &&
			mg_vcmp(&auth_domain, f_domain) == 0) {
			/* Username and domain matched, check the password */
			mg_mkmd5resp(method.p, method.len, uri.p, uri.len, f_ha1, strlen(f_ha1), nonce.p, nonce.len, nc.p, nc.len,
						 cnonce.p, cnonce.len, qop.p, qop.len, exp_resp);
			LOG(LL_DEBUG,
				("%.*s %s %.*s %s", (int)username.len, username.p, f_domain, (int)response.len, response.p, exp_resp));
...
int mg_http_is_authorized(struct http_message *hm, struct mg_str path, const char *domain, const char *passwords_file,
						  int flags)
{
...
#else
int mg_http_is_authorized(struct http_message *hm, const struct mg_str path, const char *domain,
						  const char *passwords_file, int flags)
{
	(void)hm;
	(void)path;
	(void)domain;
...
static void mg_scan_directory(struct mg_connection *nc, const char *dir, const struct mg_serve_http_opts *opts,
...
static void mg_send_directory_listing(struct mg_connection *nc, const char *dir, struct http_message *hm,
									  struct mg_serve_http_opts *opts)
{
	static const char *sort_js_code = "<script>function srt(tb, sc, so, d) {"
	...
	static const char *sort_js_code2 = "for (var i = 0; i < tr.length; i++) tb.appendChild(tr[i]); "
	...
	mg_printf_http_chunk(nc,
						 ...
						 "</thead>\n"
						 "<tbody id=tb>",
						 (int)hm->uri.len, hm->uri.p, sort_js_code, sort_js_code2, (int)hm->uri.len, hm->uri.p);
...
static int mg_http_send_port_based_redirect(struct mg_connection *c, struct http_message *hm,
											const struct mg_serve_http_opts *opts)
{
	...
	while ((rewrites = mg_next_comma_list_entry(rewrites, &a, &b)) != NULL) {
		if (mg_vcmp(&a, local_port) == 0) {
			...
			mg_printf(c, "Content-Length: 0\r\nLocation: %.*s%.*s\r\n\r\n", (int)b.len, b.p,
					  (int)(hm->proto.p - hm->uri.p - 1), hm->uri.p);
			return 1;
		}
	}
...
void mg_http_reverse_proxy(struct mg_connection *nc, const struct http_message *hm, struct mg_str mount,
						   struct mg_str upstream)
{
	...
	mg_asprintf(&purl, sizeof(burl), "%.*s%.*s", (int)upstream.len, upstream.p, (int)(hm->uri.len - mount.len),
				hm->uri.p + mount.len);

	be = mg_connect_http_base(nc->mgr, MG_CB(mg_reverse_proxy_handler, NULL), opts, "http", NULL, "https", NULL, purl,
							  &path, &user_info, &host);
...
static int mg_http_handle_forwarding(struct mg_connection *nc, struct http_message *hm,
									 const struct mg_serve_http_opts *opts)
{
	const char *rewrites = opts->url_rewrites;
	struct mg_str a, b;
...
out:
	LOG(LL_DEBUG, ("'%.*s' -> '%s' + '%.*s'", (int)hm->uri.len, hm->uri.p, *local_path ? *local_path : "",
...
static void mg_http_send_options(struct mg_connection *nc, struct mg_serve_http_opts *opts)
{
	mg_send_response_line(nc, 200, opts->extra_headers);
	mg_printf(nc, "%s",
								   ...
								   struct http_message *hm, struct mg_serve_http_opts *opts)
{
	...
	is_cgi =
		(mg_match_prefix(opts->cgi_file_pattern, strlen(opts->cgi_file_pattern), index_file ? index_file : path) > 0);

	LOG(LL_DEBUG, ("%p %.*s [%s] exists=%d is_dir=%d is_dav=%d is_cgi=%d index=%s", nc, (int)hm->method.len,
				   hm->method.p, path, exists, is_directory, is_dav, is_cgi, index_file ? index_file : ""));
	...
	if (is_dav && opts->dav_document_root == NULL) {
		mg_http_send_error(nc, 501, NULL);
	} else if (!mg_http_is_authorized(hm, mg_mk_str(path), opts->auth_domain, opts->global_auth_file,
									  ((is_directory ? MG_AUTH_FLAG_IS_DIRECTORY : 0) |
									   MG_AUTH_FLAG_IS_GLOBAL_PASS_FILE | MG_AUTH_FLAG_ALLOW_MISSING_FILE)) ||
			   !mg_http_is_authorized(hm, mg_mk_str(path), opts->auth_domain, opts->per_directory_auth_file,
									  ((is_directory ? MG_AUTH_FLAG_IS_DIRECTORY : 0) |
	...
	} else if ((!exists || mg_is_file_hidden(path, opts, 0 /* specials are ok */)) && !mg_is_creation_request(hm)) {
...
void mg_serve_http(struct mg_connection *nc, struct http_message *hm, struct mg_serve_http_opts opts)
{
	...
	if (mg_vcmp(&hm->proto, "HTTP/1.1") != 0 ||
		((hdr = mg_get_http_header(hm, "Connection")) != NULL && mg_vcmp(hdr, "keep-alive") != 0)) {
#if 0
    nc->flags |= MG_F_SEND_AND_CLOSE;
#endif
	}
										  ...
										  const char *post_data)
{
	...
	struct mbuf auth;
	struct mg_connection *nc = mg_connect_http_base(mgr, MG_CB(ev_handler, user_data), opts, "http", NULL, "https",
													NULL, url, &path, &user, &host);
	...
	mg_printf(nc, "%s %.*s HTTP/1.1\r\nHost: %.*s\r\nContent-Length: %" SIZE_T_FMT "\r\n%.*s%s\r\n%s",
			  (post_data[0] == '\0' ? "GET" : "POST"), (int)path.len, path.p, (int)(path.p - host.p), host.p,
			  strlen(post_data), (int)auth.len, (auth.buf == NULL ? "" : auth.buf), extra_headers, post_data);

	mbuf_free(&auth);
	return nc;
}

struct mg_connection *mg_connect_http(struct mg_mgr *mgr, MG_CB(mg_event_handler_t ev_handler, void *user_data),
...
size_t mg_parse_multipart(const char *buf, size_t buf_len, char *var_name, size_t var_name_len, char *file_name,
						  size_t file_name_len, const char **data, size_t *data_len)
{
...
void mg_register_http_endpoint_opt(struct mg_connection *nc, const char *uri_path, mg_event_handler_t handler,
...
static int mg_start_process(const char *interp, const char *cmd, const char *env, const char *envp[], const char *dir,
							sock_t sock)
{
	STARTUPINFOW si;
							...
							sock_t sock)
{
     ...
     */
		...
		snprintf(buf, sizeof(buf),
				 ...
				 interp == NULL ? "" : interp, interp == NULL ? "" : " ", cmd, strerror(errno));
...
static void mg_prepare_cgi_environment(struct mg_connection *nc, const char *prog, const struct mg_str *path_info,
									   const struct http_message *hm, const struct mg_serve_http_opts *opts,
									   struct mg_cgi_env_block *blk)
{
	...
	mg_addenv(blk, "REQUEST_URI=%.*s%s%.*s", (int)hm->uri.len, hm->uri.p, hm->query_string.len == 0 ? "" : "?",
	...
	for (i = 0; hm->header_names[i].len > 0; i++) {
		p = mg_addenv(blk, "HTTP_%.*s=%.*s", (int)hm->header_names[i].len, hm->header_names[i].p,
...
static void mg_send_ssi_file(struct mg_connection *nc, struct http_message *hm, const char *path, FILE *fp,
							 int include_level, const struct mg_serve_http_opts *opts);
...
static void mg_do_ssi_include(struct mg_connection *nc, struct http_message *hm, const char *ssi, char *tag,
							  int include_level, const struct mg_serve_http_opts *opts)
{
	...
	while ((ch = fgetc(fp)) != EOF) {
		if (in_ssi_tag && ch == '>' && buf[len - 1] == '-' && buf[len - 2] == '-') {
			...
			if (strncmp(p, d_include.p, d_include.len) == 0) {
				...
				mg_call(nc, NULL, nc->user_data, MG_EV_SSI_CALL, (void *)cctx.arg.p); /* NUL added above */
...
static int mg_deliver_websocket_data(struct mg_connection *nc)
{
	...
	if (ok) {
       ...
       */
			...
			/* Cleanup frame */
			memmove(nc->recv_mbuf.buf + wsd->reass_len, nc->recv_mbuf.buf + wsd->reass_len + cleanup_len,
...
void mg_send_websocket_handshake2(struct mg_connection *nc, const char *path, const char *host, const char *protocol,
								  const char *extra_headers)
{
	mg_send_websocket_handshake3(nc, path, host, protocol, extra_headers, NULL, NULL);
}

void mg_send_websocket_handshake3(struct mg_connection *nc, const char *path, const char *host, const char *protocol,
								  const char *extra_headers, const char *user, const char *pass)
{
	mg_send_websocket_handshake3v(nc, mg_mk_str(path), mg_mk_str(host), mg_mk_str(protocol), mg_mk_str(extra_headers),
								  mg_mk_str(user), mg_mk_str(pass));
}

void mg_send_websocket_handshake3v(struct mg_connection *nc, const struct mg_str path, const struct mg_str host,
								   const struct mg_str protocol, const struct mg_str extra_headers,
								   const struct mg_str user, const struct mg_str pass)
{
	struct mbuf auth;
	char key[25];
	uint32_t nonce[4];
...
void mg_send_websocket_handshake(struct mg_connection *nc, const char *path, const char *extra_headers)
{
	...
	mg_send_websocket_handshake3v(nc, mg_mk_str(path), null_str /* host */, null_str /* protocol */,
										...
										const char *extra_headers)
{
	...
	struct mg_connection *nc = mg_connect_http_base(mgr, MG_CB(ev_handler, user_data), opts, "http", "ws", "https",
													"wss", url, &path, &user_info, &host);
	if (nc != NULL) {
		mg_send_websocket_handshake3v(nc, path, host, mg_mk_str(protocol), mg_mk_str(extra_headers), user_info,
									  null_str);
	}
	return nc;
}
...
MG_INTERNAL int parse_mqtt(struct mbuf *io, struct mg_mqtt_message *mm)
{
	...
	case MG_MQTT_CMD_CONNECT: {
		...
		LOG(LL_DEBUG, ("%d %2x %d proto [%.*s] client_id [%.*s] will_topic [%.*s] "
					   "will_msg [%.*s] user_name [%.*s] password [%.*s]",
					   (int)len, (int)mm->connect_flags, (int)mm->keep_alive_timer, (int)mm->protocol_name.len,
					   mm->protocol_name.p, (int)mm->client_id.len, mm->client_id.p, (int)mm->will_topic.len,
					   mm->will_topic.p, (int)mm->will_message.len, mm->will_message.p, (int)mm->user_name.len,
					   mm->user_name.p, (int)mm->password.len, mm->password.p));
		break;
	}
...
void mg_mqtt_publish(struct mg_connection *nc, const char *topic, uint16_t message_id, int flags, const void *data,
					 size_t len)
{
	uint16_t netbytes;
...
void mg_mqtt_subscribe(struct mg_connection *nc, const struct mg_mqtt_topic_expression *topics, size_t topics_len,
					   uint16_t message_id)
{
	uint16_t netbytes;
	size_t i;
...
static void mg_mqtt_broker_handle_subscribe(struct mg_connection *nc, struct mg_mqtt_message *msg)
{
	...
	if (num_subs > 0) {
		te = (struct mg_mqtt_topic_expression *)MG_REALLOC(ss->subscriptions, sizeof(*ss->subscriptions) *
		...
		ss->subscriptions = te;
		for (pos = 0; pos < (int)msg->payload.len && (pos = mg_mqtt_next_subscribe_topic(msg, &topic, &qos, pos)) != -1;
...
struct mg_dns_resource_record *mg_dns_next_record(struct mg_dns_message *msg, int query,
...
int mg_dns_encode_record(struct mbuf *io, struct mg_dns_resource_record *rr, const char *name, size_t nlen,
...
static unsigned char *mg_parse_dns_resource_record(unsigned char *data, unsigned char *end,
...
int mg_dns_reply_record(struct mg_dns_reply *reply, struct mg_dns_resource_record *question, const char *name,
						int rtype, int ttl, const void *rdata, size_t rdata_len)
{
	...
	if ((err = RegOpenKeyExW(HKEY_LOCAL_MACHINE, key, 0, KEY_READ, &hKey)) != ERROR_SUCCESS) {
		...
		for (ret = -1, i = 0; 1; i++) {
			...
			if (RegOpenKeyExW(hKey, subkey, 0, KEY_READ, &hSub) == ERROR_SUCCESS &&
				((RegQueryValueExW(hSub, L"NameServer", 0, &type, (void *)value, &len) == ERROR_SUCCESS &&
				  value[0] != '\0') ||
				 (RegQueryValueExW(hSub, L"DhcpNameServer", 0, &type, (void *)value, &len) == ERROR_SUCCESS &&
				  value[0] != '\0'))) {
				/*
...
int mg_resolve_async_opt(struct mg_mgr *mgr, const char *name, int query, mg_resolve_callback_t cb, void *data,
						 struct mg_resolve_async_opts opts)
{
	struct mg_resolve_async_request *req;
...
static uint32_t coap_calculate_packet_size(struct mg_coap_message *cm, size_t *len)
{
	...
	*len += cm->token.len;
     ...
     */
		if ((opt->next != NULL && opt->number > opt->next->number) || opt->value.len > 0xFFFF + 269 ||
...
struct mg_connection *mg_sntp_connect(struct mg_mgr *mgr, MG_CB(mg_event_handler_t event_handler, void *user_data),
...
struct mg_connection *mg_sntp_get_time(struct mg_mgr *mgr, mg_event_handler_t event_handler,
									   const char *sntp_server_name)
{
...
static void mg_socks5_handle_request(struct mg_connection *c)
{
	...
	if (ver != MG_SOCKS_VERSION || cmd != MG_SOCKS_CMD_CONNECT) {
		...
		if (r->len < (size_t)addr_len + 6)
			return; /* return if not buffered */
		snprintf(addr, sizeof(addr), "[%x:%x:%x:%x:%x:%x:%x:%x]:%d", p[4] << 8 | p[5], p[6] << 8 | p[7],
				 p[8] << 8 | p[9], p[10] << 8 | p[11], p[12] << 8 | p[13], p[14] << 8 | p[15], p[16] << 8 | p[17],
				 p[18] << 8 | p[19], p[20] << 8 | p[21]);
		mg_socks5_connect(c, addr);
...
static struct sl_file_open_info *fs_slfs_find_foi(const char *name, bool create);
...
int fs_slfs_open(const char *pathname, int flags, mode_t mode)
{
   ...
   */
	...
	int rw = (flags & 3);
	size_t new_size = 0;
	struct sl_file_open_info *foi = fs_slfs_find_foi(pathname, false /* create */);
	if (foi != NULL) {
		LOG(LL_DEBUG, ("FOI for %s: %d 0x%x %p", pathname, (int)foi->size, (unsigned int)foi->flags, foi->token));
	}
	if (rw == O_RDONLY) {
	...
	LOG(LL_DEBUG,
		("sl_FsOpen(%s, 0x%x, %p) sz %u = %d", pathname, (int)am, token, (unsigned int)new_size, (int)fi->fh));
	int r;
	if (fi->fh >= 0) {
		fi->pos = 0;
		r = fd;
...
static struct sl_file_open_info *fs_slfs_find_foi(const char *name, bool create)
{
	...
	for (i = 0; i < MAX_OPEN_SLFS_FILES; i++) {
		if (s_sl_file_open_infos[i].name != NULL && strcmp(drop_dir(s_sl_file_open_infos[i].name, NULL), name) == 0) {
			break;
		}
	}
...
#if MG_NET_IF == MG_NET_IF_SIMPLELINK && (defined(MG_FS_SLFS) || defined(MG_FS_SPIFFS))
...
#pragma diag_suppress 169 /* Nothing we can do about the prototype mismatch. \
                             */
	ret = (add_device("SL", _MSA, fs_slfs_open, fs_slfs_close, fs_slfs_read, fs_slfs_write, fs_slfs_lseek,
					  fs_slfs_unlink, fs_slfs_rename) == 0);
#pragma diag_pop
#endif
#endif
	return ret;
...
const char *inet_ntop(int af, const void *src, char *dst, socklen_t size)
{
	...
	res = snprintf(dst, size, "%lu.%lu.%lu.%lu", SL_IPV4_BYTE(in->s_addr, 0), SL_IPV4_BYTE(in->s_addr, 1),
				   SL_IPV4_BYTE(in->s_addr, 2), SL_IPV4_BYTE(in->s_addr, 3));
...
out:
	DBG(("%p to %s:%d sock %d %d err %d", nc, inet_ntoa(sa->sin.sin_addr), ntohs(sa->sin.sin_port), nc->sock, proto,
		 nc->err));
}
...
void mg_mgr_handle_conn(struct mg_connection *nc, int fd_flags, double now)
{
	DBG(("%p fd=%d fd_flags=%d nc_flags=0x%lx rmbl=%d smbl=%d", nc, nc->sock, fd_flags, nc->flags,
		 (int)nc->recv_mbuf.len, (int)nc->send_mbuf.len));
	...
	DBG(("%p after fd=%d nc_flags=0x%lx rmbl=%d smbl=%d", nc, nc->sock, nc->flags, (int)nc->recv_mbuf.len,
		 (int)nc->send_mbuf.len));
}
   ...
   */
	...
	for (nc = mgr->active_connections, num_fds = 0; nc != NULL; nc = tmp) {
		...
		if (nc->sock != INVALID_SOCKET) {
			...
			if (!(nc->flags & MG_F_WANT_WRITE) && nc->recv_mbuf.len < nc->recv_mbuf_limit &&
		...
		if (nc->sock != INVALID_SOCKET) {
			if (num_ev > 0) {
				fd_flags = (SL_SOCKET_FD_ISSET(nc->sock, &read_set) && (!(nc->flags & MG_F_UDP) || nc->listener == NULL)
								? _MG_F_FD_CAN_READ
								: 0) |
						   (SL_SOCKET_FD_ISSET(nc->sock, &write_set) ? _MG_F_FD_CAN_WRITE : 0) |
   ...
   */
	...
	for (nc = mg_next(mgr, NULL); nc != NULL; nc = mg_next(mgr, nc)) {
		...
		if (nc->flags & MG_F_LISTENING) {
			...
			int res = (nc->flags & MG_F_UDP ? mg_sl_if_listen_udp(nc, &nc->sa) : mg_sl_if_listen_tcp(nc, &nc->sa));
			if (res == 0)
				continue;
...
#define MG_SSL_IF_SIMPLELINK_SLFS_PREFIX_LEN (sizeof(MG_SSL_IF_SIMPLELINK_SLFS_PREFIX) - 1)
...
static char *sl_pem2der(const char *pem_file)
{
	...
	if (result) {
		...
		memmove(der_file, der_file + MG_SSL_IF_SIMPLELINK_SLFS_PREFIX_LEN, l - 2 /* including \0 */);
	} else {
		MG_FREE(der_file);
		der_file = NULL;
	}
...
int sl_set_ssl_opts(int sock, struct mg_connection *nc)
{
	...
	if (ctx == NULL)
		return 0;
	DBG(("%p %s,%s,%s,%s", nc, (ctx->ssl_cert ? ctx->ssl_cert : "-"), (ctx->ssl_key ? ctx->ssl_cert : "-"),
		 (ctx->ssl_ca_cert ? ctx->ssl_ca_cert : "-"), (ctx->ssl_server_name ? ctx->ssl_server_name : "-")));
	if (ctx->ssl_cert != NULL && ctx->ssl_key != NULL) {
		char *ssl_cert = sl_pem2der(ctx->ssl_cert), *ssl_key = NULL;
		if (ssl_cert != NULL) {
			err = sl_SetSockOpt(sock, SL_SOL_SOCKET, SL_SO_SECURE_FILES_CERTIFICATE_FILE_NAME, ssl_cert,
								strlen(ssl_cert));
			MG_FREE(ssl_cert);
			LOG(LL_DEBUG, ("CERTIFICATE_FILE_NAME %s -> %d", ssl_cert, err));
			ssl_key = sl_pem2der(ctx->ssl_key);
			if (ssl_key != NULL) {
				err = sl_SetSockOpt(sock, SL_SOL_SOCKET, SL_SO_SECURE_FILES_PRIVATE_KEY_FILE_NAME, ssl_key,
									strlen(ssl_key));
				MG_FREE(ssl_key);
	...
	if (ctx->ssl_ca_cert != NULL) {
		if (ctx->ssl_ca_cert[0] != '\0') {
			...
			if (ssl_ca_cert != NULL) {
				err = sl_SetSockOpt(sock, SL_SOL_SOCKET, SL_SO_SECURE_FILES_CA_FILE_NAME, ssl_ca_cert,
									strlen(ssl_ca_cert));
	...
	if (ctx->ssl_server_name != NULL) {
		err = sl_SetSockOpt(sock, SL_SOL_SOCKET, SL_SO_SECURE_DOMAIN_NAME_VERIFICATION, ctx->ssl_server_name,
...
#define SET_ADDR(dst, src) memcpy((dst)->sin6.sin6_addr.s6_addr, (src)->ip6.addr, sizeof((dst)->sin6.sin6_addr.s6_addr))
#else
...
{
	struct mg_lwip_netif_tcpip_call_ctx *ctx = (struct mg_lwip_netif_tcpip_call_ctx *)arg;
	ctx->fn(ctx->arg);
...
static err_t mg_lwip_tcp_sent_cb(void *arg, struct tcp_pcb *tpcb, u16_t num_sent)
{
	...
	if (nc == NULL)
		return ERR_OK;
	if ((nc->flags & MG_F_SEND_AND_CLOSE) && !(nc->flags & MG_F_WANT_WRITE) && nc->send_mbuf.len == 0 &&
...
static int mg_lwip_if_udp_recv(struct mg_connection *nc, void *buf, size_t len, union socket_address *sa,
							   size_t *sa_len)
{
	/*
...
static err_t mg_lwip_accept_cb(void *arg, struct tcp_pcb *newtpcb, err_t err)
{
	...
	LOG(LL_DEBUG,
		("%p conn %p from %s:%u", lc, newtpcb, IPADDR_NTOA(ipX_2_ip(&newtpcb->remote_ip)), newtpcb->remote_port));
	if (lc == NULL) {
		tcp_abort(newtpcb);
		return ERR_ABRT;
	}
...
{
	LOG(LL_INFO,
		("Mongoose %s, LwIP %u.%u.%u", MG_VERSION, LWIP_VERSION_MAJOR, LWIP_VERSION_MINOR, LWIP_VERSION_REVISION));
...
time_t mg_lwip_if_poll(struct mg_iface *iface, int timeout_ms)
{
  ...
  DBG(("begin poll @%u", (unsigned int) (now * 1000)));
	...
	for (nc = mgr->active_connections; nc != NULL; nc = tmp) {
		...
		if (!mg_if_poll(nc, now))
			continue;
		if (nc->sock != INVALID_SOCKET && !(nc->flags & (MG_F_UDP | MG_F_LISTENING)) && cs->pcb.tcp != NULL &&
...
int _wstati64(const wchar_t *path, cs_stat_t *st)
{
	...
	if ((fa & FILE_ATTRIBUTE_DIRECTORY) == 0) {
		...
		st->st_mtime = (uint32_t)(
			(((uint64_t)ftime.dwLowDateTime + ((uint64_t)ftime.dwHighDateTime << 32)) / 10000000.0) - 11644473600);
		CloseHandle(h);
	} else {
		st->st_mode |= _S_IFDIR;
	}
...
static void mg_gmt_time_string(char *buf, size_t buf_len, time_t *t)
{
	...
	snprintf(buf, buf_len, "%d.%d.%d %d:%d:%d GMT", (int)systime.wYear, (int)systime.wMonth, (int)systime.wDay,
...
{
	nc->sock = TCPIP_UDP_ServerOpen(sa->sin.sin_family == AF_INET ? IP_ADDRESS_TYPE_IPV4 : IP_ADDRESS_TYPE_IPV6,
									ntohs(sa->sin.sin_port),
...
{
	nc->sock = TCPIP_TCP_ServerOpen(sa->sin.sin_family == AF_INET ? IP_ADDRESS_TYPE_IPV4 : IP_ADDRESS_TYPE_IPV6,
									ntohs(sa->sin.sin_port),
									sa->sin.sin_addr.s_addr == 0 ? 0 : (IP_MULTI_ADDRESS *)&sa->sin);
...
{
	static char addr[17];
	snprintf(addr, sizeof(addr), "%d.%d.%d.%d", (int)in.S_un.S_un_b.s_b1, (int)in.S_un.S_un_b.s_b2,
			 (int)in.S_un.S_un_b.s_b3, (int)in.S_un.S_un_b.s_b4);
	return addr;
}
...
static void mg_handle_send(struct mg_connection *nc)
{
	...
	if (nc->flags & MG_F_UDP) {
		if (!TCPIP_UDP_RemoteBind((UDP_SOCKET)nc->sock,
								  nc->sa.sin.sin_family == AF_INET ? IP_ADDRESS_TYPE_IPV4 : IP_ADDRESS_TYPE_IPV6,
		...
		if (bytes_written >= nc->send_mbuf.len) {
			if (TCPIP_UDP_ArrayPut((UDP_SOCKET)nc->sock, (uint8_t *)nc->send_mbuf.buf, nc->send_mbuf.len) !=
				nc->send_mbuf.len) {
				nc->flags |= MG_F_CLOSE_IMMEDIATELY;
				bytes_written = 0;
			}
		...
		if (bytes_written != 0) {
			...
			if (TCPIP_TCP_ArrayPut((TCP_SOCKET)nc->sock, (uint8_t *)nc->send_mbuf.buf, bytes_written) !=
				bytes_written) {
				nc->flags |= MG_F_CLOSE_IMMEDIATELY;
				bytes_written = 0;
			}
...
{
	nc->sock = TCPIP_TCP_ClientOpen(sa->sin.sin_family == AF_INET ? IP_ADDRESS_TYPE_IPV4 : IP_ADDRESS_TYPE_IPV6,
